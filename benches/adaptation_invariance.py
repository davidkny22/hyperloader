"""Run the installed schedule-invariance verifier and preserve raw evidence."""
...
def main() -> None:
    ...
    include = ",".join(
        [
            ...
            "*/tests/hyperloader/test_adaptation_invariance.py",
        ]
    )
    verification = _run(
        [
            ...
            "-m",
            "unittest",
            "tests.hyperloader.test_adaptation_invariance",
            "-v",
        ],
        root,
        environment,
    ...
    mutation_environment["HYPERLOADER_ADAPTATION_MUTATION"] = "freeze-controller"
    mutation = _run(
        [
            sys.executable,
            "-m",
    ...
    if mutation.returncode == 0:
        raise RuntimeError("the frozen-controller mutation did not make the suite fail")
    ...
    summary = {
        ...
        "negative": "changed root seed changed the stream",
        "python": sys.version,
