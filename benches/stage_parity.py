"""Run the installed stage-parity verifier and preserve raw evidence."""
...
def main() -> None:
    """Run installed parity, a changed-seed negative, and a wiring mutation."""
    ...
    include = ",".join(
        [
            "*/hyperloader/api.py",
            "*/hyperloader/planner/stages.py",
            "*/hyperloader/stages/*.py",
            "*/tests/hyperloader/stages/test_pipeline.py",
            "*/tests/hyperloader/test_stage_parity.py",
        ]
    )
    modules = [
        "tests.hyperloader.stages.test_pipeline",
        "tests.hyperloader.test_stage_parity",
    ]
    verification = _run(
        [
            sys.executable,
    ...
    mutation_environment["HYPERLOADER_STAGE_PARITY_MUTATION"] = "skip-sample-chain"
    mutation = _run(
    ...
    if mutation.returncode == 0:
        raise RuntimeError(
            "the severed sample-stage wiring did not make the suite fail"
        )
    ...
    summary = {
        ...
        "negative": "changed root seed changed the staged stream",
        "python": sys.version,
